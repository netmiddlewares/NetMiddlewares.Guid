...
namespace NetMiddlewares.Guid.Tests
{
    public class GuidMiddlewareTests
    {
        [Fact]
        public async Task TestGuidMiddlewareAddsHeaderCorrectly()
        {
            // Arrange
            (var context, var next) = GetOnStartingResponseAwareContext();
            GuidMiddleware mw = new GuidMiddleware();
            ...
            Assert.True(context.Response.Headers.TryGetValue("X-Guid", out var value1));
